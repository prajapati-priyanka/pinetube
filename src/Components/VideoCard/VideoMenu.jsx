...
const VideoMenu = ({setIsPlaylistModalVisible, setIsVideoMenuVisible, videos})=>{
  const {authState} = useAuth();
  const {likeDispatch, likeState:{likeData}} = useLike();
 ...
 const likeClickHandler=()=>{
    ...
    return(
      <>
       ...
       <button className="btn menu-row flex-container" onClick={likeClickHandler}>
          <AiOutlineLike className="menu-row-icon lg-text" />
          <span className="icon-name md-text">{(isVideoInLikePage && token) ? "Remove from" : "Add To"} liked Videos</span>
        </button>
